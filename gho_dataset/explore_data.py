...
def explore_raw(rawfile=year+'dataset_raw.csv'):
    ...
    valcounts.to_csv(year+'dataset_statuscols.csv')
    return


def get_missing_countries(uselist=0, countrylist=year+'ghocountries.csv'):
    ...
    df1 = pd.read_csv(countrylist, header=None,
                      names=['code', 'name'])
    ...
    missingcountries.to_csv('missingcountries.csv')
    return missingcountries


def get_missing_values():
    df = pd.read_csv(year + 'dataset_clean.csv')
    nullcount = []
    ...
    dfnulls.to_csv(year + 'dataset_columnnulls.csv')

    rownulls = pd.DataFrame(df.isnull().sum(axis=1), columns=['nulls'])
    rownulls['country'] = df['meta_country']
    rownulls.to_csv(year + 'dataset_rownulls.csv', columns=['country', 'nulls'])
    return
